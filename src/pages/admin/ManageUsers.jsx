...
//                 {user.authorizationStatus === "approved" && currentUser?.role !== "dunis_admin" && (
//                   <button className="icon-action" disabled={user.id === currentUser?.id}
//                     onClick={() => updateUser(user, { authorizationStatus: "pending", isAuthorized: false })} 
//                     type="button">
...
//                   <button className="icon-action" disabled={user.id === currentUser?.id}
//                     onClick={() => updateUser(user, { authorizationStatus: "removed", isAuthorized: false, isActive: false })} 
//                     type="button">
...
//                 {user.authorizationStatus === "removed" && currentUser?.role !== "saed_admin" && (
//                   <button className="icon-action" disabled={user.id === currentUser?.id}
//                     onClick={() => updateUser(user, { authorizationStatus: "pending", isAuthorized: false, isActive: true })}
//                     type="button">
...
// `authorizationStatus` is the single source of truth for a trainer's
// review state (pending / approved / declined / removed). `isAuthorized`
// on the API response is a derived read-only flag kept in sync with it
// server-side -- it is never sent back up as an independent value from
// here, since doing so previously let it drift out of sync with
// authorizationStatus (e.g. the "Approved" filter tab counted by status
// while the tab's own filtering checked isAuthorized).
const emptyEditForm = {
  fullName: "", phone: "", specialization: "", yearsExperience: "",
  companyName: "", bio: "", partnerLgas: [], numberTrained: "",
};
...
export default function ManageUsers() {
  ...
  const trainers = users.filter((u) => u.role === "trainer");
  // All branches key off authorizationStatus -- the single source of
  // truth -- rather than mixing in the derived isAuthorized flag, so the
  // filter-tab counts and the actual filtered list can never disagree.
  const filtered = filter === "all" ? trainers : trainers.filter((u) => u.authorizationStatus === filter);
    ...
    <section className="panel full-panel">
      ...
      {!loading ? (
        <>
          ...
          {paginatedUsers.map((user) => {
            const authStatus = user.authorizationStatus;
            ...
            <div className="management-row" key={user.id}>
              ...
              <span>{user.specialization || "—"}</span>
              <span className={`status-pill status-${user.isActive ? "approved" : "declined"}`}>{user.isActive ? "active" : "inactive"}</span>
              <span className={`status-pill status-${authStatus === "approved" ? "approved" : authStatus === "declined" ? "declined" : authStatus === "removed" ? "declined" : "pending"}`}>
                {statusLabels[authStatus] || "Pending"}
              </span>
              <div className="action-buttons">
                <button className="icon-action" onClick={() => setViewUser(user)} type="button" title="View">
                  <Eye size={16} /> <span>View</span>
                ...
                {/* Authorization decisions: available to any admin now that
                    dunis_admin is a superset of saed_admin. Approve must
                    still exclude "removed" -- a removed account has to go
                    through Restore first so it gets a fresh review, rather
                    than being re-approved directly while still deactivated.
                    The backend enforces this transition rule independently. */}
                {authStatus !== "approved" && authStatus !== "removed" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { authorizationStatus: "approved" })} type="button">
                    <ShieldCheck size={16} /> <span>Approve</span>
                  </button>
                )}
                {authStatus === "approved" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { authorizationStatus: "pending" })}
                    type="button">
                    <ShieldOff size={16} /> <span>Disapprove</span>
                  </button>
                )}
                {authStatus !== "declined" && authStatus !== "removed" && authStatus !== "approved" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { authorizationStatus: "declined" })} type="button">
                    <ShieldOff size={16} /> <span>Decline</span>
                  </button>
                )}
                {/* A declined trainer previously had no way back to review
                    short of a blind Approve or a full Remove. This sends
                    them back to "pending" so they go through normal review
                    again -- e.g. after resubmitting documents. */}
                {authStatus === "declined" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { authorizationStatus: "pending" })} type="button">
                    <RefreshCw size={16} /> <span>Re-review</span>
                  </button>
                )}
                {authStatus !== "removed" && currentUser?.role !== "saed_admin" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { authorizationStatus: "removed" })}
                    type="button">
                    <Trash2 size={16} /> <span>Remove</span>
                  </button>
                )}
                {/* Restore always lands on "pending", never back on the
                    account's pre-removal status -- removal can happen from
                    approved, pending, or declined, and we deliberately don't
                    remember which, so a restored trainer always gets a
                    fresh review rather than silently regaining approval. */}
                {authStatus === "removed" && currentUser?.role !== "saed_admin" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { restore: true })}
                    type="button">
                    <RefreshCw size={16} /> <span>Restore</span>
                  </button>
                )}

                {/* Independent of the authorization workflow: a temporary
                    login on/off switch that leaves authorizationStatus
                    untouched (e.g. suspend an approved trainer without
                    revoking their approval). Hidden entirely once a trainer
                    is "removed" -- reactivating a removed account has to go
                    through Restore, which also forces a fresh review;
                    otherwise you could end up with isActive=true while
                    authorizationStatus is still "removed". */}
                {authStatus !== "removed" && currentUser?.role !== "saed_admin" && (
                  <button className="icon-action" disabled={isSelf}
                    onClick={() => updateUser(user, { isActive: !user.isActive })}
                    title={isSelf ? "You cannot deactivate your own account" : "Suspends login only; does not change authorization status"} type="button">
                    <Save size={16} />
                    <span>{isSelf ? "Current User" : user.isActive ? "Deactivate" : "Activate"}</span>
                  </button>
                )}
              </div>
            </div>
