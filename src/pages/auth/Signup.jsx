...
// import {
//     RoleSelector,
//     FormField,FormRow, SubmitButton,
//      StepIndicator, TermsCheckbox
...
//   const [partnerLgas, toggleLga, setLgas] = useToggleArray();
...
//     if (validateUsername(activeForm.form.username)) errors.username = validateUsername(activeForm.form.username);
//     if (validateEmail(activeForm.form.email)) errors.email = validateEmail(activeForm.form.email);
//     if (validatePhone(activeForm.form.phone)) errors.phone = validatePhone(activeForm.form.phone);
//     if (validateNyscCode(activeForm.form.nyscStateCode)) errors.nyscStateCode = validateNyscCode(activeForm.form.nyscStateCode);
//     if (validateRequired(activeForm.form.lgaOfDeployment, MESSAGES.LGA_REQUIRED)) errors.lgaOfDeployment = MESSAGES.LGA_REQUIRED;
...
//     const errors = {};
//     if (validateRequired(activeForm.form.skillInterest, MESSAGES.SKILL_REQUIRED)) errors.skillInterest = MESSAGES.SKILL_REQUIRED;
//     return errors;
//   }
...
//     if (validateEmail(activeForm.form.email)) errors.email = validateEmail(activeForm.form.email);
//     if (validatePhone(activeForm.form.phone)) errors.phone = validatePhone(activeForm.form.phone);
//     if (validateRequired(activeForm.form.specialization, MESSAGES.SPECIALIZATION_REQUIRED)) errors.specialization = MESSAGES.SPECIALIZATION_REQUIRED;
//     if (validateRequired(partnerLgas, MESSAGES.LGA_MIN)) errors.partnerLgas = MESSAGES.LGA_MIN;
//     if (validateRequired(activeForm.form.partnershipLetter, MESSAGES.PARTNERSHIP_REQUIRED)) errors.partnershipLetter = MESSAGES.PARTNERSHIP_REQUIRED;
//     const pwErrors = validatePassword(activeForm.form.password, activeForm.form.confirmPassword);
//     Object.assign(errors, pwErrors);
//     return errors;
//   }
...
//     try {
//       await signup({ ...activeForm.form, role: "corps_member", stateOfDeployment: "Lagos" });
//       navigate("/app");
//     } catch (err) {
...
//       {(step === 1 || isTrainer) && (
//         <RoleSelector value={role} onChange={(r) => { setRole(r); setStep(1); }} />
//       )}

//       <form className="auth-form" onSubmit={handleSubmit}>
//         {isTrainer ? <TrainerFields form={activeForm} lgas={partnerLgas} toggleLga={toggleLga} /> : (
//           step === 1 ? <CorpsStep1 form={activeForm} /> : <CorpsStep2 form={activeForm} />
//         )}
...
//             <SubmitButton loading={activeForm.submitting} disabled={!agree}>
//               {isTrainer ? "Create Trainer Account →" : "Continue →"}
//             </SubmitButton>
//           </>
//         ) : (
//           <SubmitButton loading={activeForm.submitting}>Continue to Next Step →</SubmitButton>
//         )}
...
//         ))}
//       </div>
//       {form.fields.skillInterest && <span className="field-error">{form.fields.skillInterest}</span>}
//     </>
//   );
// }
...
//           onChange={form.update}
//           placeholder="Select"
//           options={EXPERIENCE_YEARS.map((y) => ({ value: String(y), label: `${y} year${y > 1 ? "s" : ""}` }))}
//         />
//         <FormField
//           label="Company Name"
//           name="companyName"
...
//         </div>
//       </label>
//       {form.fields.partnerLgas && <span className="field-error">{form.fields.partnerLgas}</span>}
...
export default function Signup() {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            corpsForm.setSubmitError(err);
            console.error("Corps signup error:", err);
        } finally {
            corpsForm.endSubmit();
        }
    }
